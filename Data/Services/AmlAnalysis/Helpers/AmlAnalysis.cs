...
namespace Data.Services.AmlAnalysis
{
    public class AmlAnalysis : IAmlAnalysis
    {
        ...
        public async Task<(
            bool isSucceed,
            IEnumerable<string>? ColseFailedEntities
        )> CloseAlertsAsync(CloseRequest closeReq, string userName, string alertStatusCode)
        {
            try
            {
                (bool isSucceed, IEnumerable<decimal>? AlertsIds) = await SetAlertStatus(
                    closeReq.Entities
                );
                if (!isSucceed)
                {
                ...
                if (!commentsRes)
                    return ((bool isSucceed, IEnumerable<string>? ColseFailedEntities))
                        (false, closeReq.Entities);
            ...
            {
                _logger.LogCritical(
                    "Something wrong happend while closing : {exception}",
                    ex.Message
                );
                return (false, closeReq.Entities);
            }
        ...
        public async Task<(
            bool isSucceed,
            IEnumerable<string>? ColseFailedEntities
        )> RouteAlertsAsync(RouteRequest routeReq, string userName, string desc = "RTQ")
        {
            try
            {
                ...
                bool isCreateEventsSucceed = await CreateEntityEvents(
                    routeReq.Entities,
                    desc,
                    $@"{routeReq.QueueCode}--{routeReq.OwnerId}",
                    userName
                );
                ...
                bool isAddCommentsSucceed = await AddComments(
                    routeReq.Entities,
                    routeReq.Comment,
                    userName
                );
            ...
            {
                _logger.LogCritical(
                    "Something wrong happend while routing : {exception}",
                    ex.Message
                );
                return (false, routeReq.Entities);
            }
        ...
        private async Task<bool> CreateEntityEvents(
            IEnumerable<string> entitiesNumber,
        ...
        )
        {
            ...
            IEnumerable<FskEntityEvent> events = entitiesNumber.Select(a => new FskEntityEvent
            {
                ...
                EntityLevelCode =
                    _fcfkc
                        ?.FskAlertedEntities?.FirstOrDefault(x => x.AlertedEntityNumber == a)
        ...
        private async Task<bool> SaveEntityQueue(
        ...
        )
        {
            try
            {
                IQueryable<string> existingQueues = _fcfkc
                    .FskEntityQueues.Where(q => entities.Contains(q.AlertedEntityNumber))
                    .Select(q => q.AlertedEntityNumber);
                ...
                {
                    string sqlServerQeury =
                        "SELECT TOP(1) alerted_entity_level_code FROM FCFKC.FSK_ALERTED_ENTITY a WHERE a.alerted_entity_number = alerted_entity_number";
                    string oracleQeury =
                        "SELECT alerted_entity_level_code FROM FCFKC.FSK_ALERTED_ENTITY a WHERE a.alerted_entity_number = alerted_entity_number FETCH FIRST  ROW ONLY";
                    string sql = _fcfkc.Database.IsOracle() ? oracleQeury : sqlServerQeury;
                    int updateRes = await _fcfkc.Database.ExecuteSqlRawAsync(
                        $@"UPDATE FCFKC.FSK_ENTITY_QUEUE
                ...
                IEnumerable<FskEntityQueue>? newEntitiesQueues = newQueuesAlertedNumbers?.Select(
                    a => new FskEntityQueue
                    {
                        AlertedEntityLevelCode =
                            _fcfkc
                                ?.FskAlertedEntities?.FirstOrDefault(x =>
                                    x.AlertedEntityNumber == a
                                )
                                ?.AlertedEntityLevelCode ?? "",
                ...
                {
                    int res = await _fcfkc.Database.ExecuteSqlRawAsync(
                        $@"INSERT INTO FCFKC.FSK_ENTITY_QUEUE(alerted_entity_level_code , alerted_entity_number , owner_userid , queue_code)
            ...
            {
                _logger.LogCritical(
                    "Somthing Went Wrong when setting entity queues: {Exception}",
                    ex.Message
                );
                return false;
            }
        ...
        private async Task<bool> AddComments(
        ...
        private async Task<bool> ClearAlertsCount(IEnumerable<string> alertedEntities)
        {
            try
            {
                int rowEffected = await _fcfkc.Database.ExecuteSqlRawAsync(
                    $@"UPDATE FCFKC.FSK_ALERTED_ENTITY 
        ...
        private async Task<(bool isSucceed, IEnumerable<decimal>? AlertsIds)> SetAlertStatus(
            IEnumerable<string> alertedEntities,
            string Status = "CLP"
        )
        {
            try
            {
                ...
                IQueryable<decimal> alertIds = _fcfkc
                    .FskAlerts.Where(x =>
                        alertedEntities.Contains(x.AlertedEntityNumber)
                        && x.AlertStatusCode.ToLower() == "act"
                    )
                    .Select(x => x.AlertId);
                if (alertIds.Count() <= 0)
                {
                    _logger.LogCritical(
                        "there is no alerts with 'CLP' on this entites ({AEN})",
                        string.Join(",", alertedEntities)
                    );
                    return (false, null);
                }
        ...
        public async Task<(
            bool isSucceed,
            IEnumerable<string>? ColseFailedEntities
        )> CloseAllAlertsAsync(CloseRequest closeRequest, string userName, string alertStatusCode)
        {
            IEnumerable<CloseRequest> requests = Partition(closeRequest.Entities, 1000)
                .Select(x => new CloseRequest
                {
                    Entities = x,
                    ...
                    foreach (CloseRequest request in requests)
                    {
                        ...
                        if (!deleteFromBkRes)
                            throw new InvalidOperationException(
                                $"Error While deleting entities from Table : ({string.Join(",", request.Entities)})"
                            );
                    }
                    arTrans.Commit();
                    trans.Commit();
        ...
        public async Task<(
            bool isSucceed,
            IEnumerable<string>? RouteFailedEntities
        )> RouteAllAlertsAsync(RouteRequest routeReq, string userName, string desc = "RTQ")
        {
            IEnumerable<RouteRequest> requests = Partition(routeReq.Entities, 1000)
                .Select(x => new RouteRequest
                {
                    Entities = x,
     ...
     execute immediate 'CREATE TABLE ART_AML_ANALYSIS_VIEW_TB AS SELECT * FROM ART_AML_ANALYSIS_VIEW';
            ...
            try
            {
                if (!_updateInd.PerformInd)
                    throw new InvalidOperationException(
                        "the update indecator is false can't call create aml analysis table"
                    );
            ...
            {
                _logger.LogCritical(
                    "something wrong jappend while creating aml analysis table : {ex}",
                    ex.Message
                );
                return false;
            }
        ...
        {
            IQueryable<ArtAmlAnalysisRule> closeRules = _context.ArtAmlAnalysisRules.Where(x =>
                x.Active && !x.Deleted && x.Action == AmlAnalysisAction.Close.ToString()
            );
            ...
            {
                List<string> AENs = _context
                    .ArtAmlAnalysisViewTbs.FromSqlRaw(
                        $"Select * From {rule.TableName} Where {rule.Sql}"
                    )
                    .Select(a => a.PartyNumber)
                    .ToList();
                closeEntities.AddRange(AENs);
                (bool isSucceed, IEnumerable<string>? ColseFailedEntities) =
                    await CloseAllAlertsAsync(
                        new CloseRequest
                        {
                            Entities = AENs,
                ...
                if (!isSucceed)
                    _logger.LogCritical(
                        "rule number {rule} failed cause this entities caused an issue : ({AENS})",
                        rule.Id,
                        string.Join(",", ColseFailedEntities)
                    );
            }

            IQueryable<ArtAmlAnalysisRule> routeRules = _context.ArtAmlAnalysisRules.Where(x =>
                x.Active && !x.Deleted && x.Action == AmlAnalysisAction.Route.ToString()
            );
            foreach (ArtAmlAnalysisRule rule in routeRules)
            {
                IEnumerable<string> ruleEntities = _context
                    .ArtAmlAnalysisViewTbs.FromSqlRaw(
                        $"Select * From {rule.TableName} Where {rule.Sql}"
                    )
                    .Select(a => a.PartyNumber)
                    .ToList()
                    .Where(x => !closeEntities.Contains(x));
                string[] queue_owner = rule.RouteToUser.Split("--");

                (bool isSucceed, IEnumerable<string>? RouteFailedEntities) =
                    await RouteAllAlertsAsync(
                        new RouteRequest
                        {
                ...
                if (!isSucceed)
                    _logger.LogCritical(
                        "rule number {rule} failed cause this entities caused an issue : ({AENS})",
                        rule.Id,
                        string.Join(",", RouteFailedEntities)
                    );
            }
        }
        ...
        public async Task<IEnumerable<TestRulesResult>> TestRules(IEnumerable<int> rules)
        {
            ...
            {
                List<string> aens = _context
                    .ArtAmlAnalysisViewTbs.FromSqlRaw($"Select * From {x.TableName} Where {x.Sql}")
                    .Select(x => x.PartyNumber)
                    .ToList();
        ...
        )
        {
            ...
            IEnumerable<ArtAmlAnalysisRule> closeRules = _context.ArtAmlAnalysisRules.Where(x =>
                x.Active
                && !x.Deleted
                && x.Action == AmlAnalysisAction.Close.ToString()
                && rules.Contains(x.Id)
            ).ToList();
            List<string> closeEntities = new();
            List<(bool isSucceed, IEnumerable<string>? FailedEntities)> failedRules = new();
            ...
            IQueryable<ArtAmlAnalysisRule> routeRules = _context.ArtAmlAnalysisRules.Where(x =>
                x.Active
                && !x.Deleted
                && x.Action == AmlAnalysisAction.Route.ToString()
                && rules.Contains(x.Id)
            );
            foreach (ArtAmlAnalysisRule rule in routeRules)
            {
