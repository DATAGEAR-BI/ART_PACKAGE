...
namespace OracleMigrations.Migrations
{
    public partial class FTI_Reports : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            //ART_TI_ADVANCE_PAYMENT_UTILIZATION
            migrationBuilder.Sql($@"
             CREATE OR REPLACE FORCE NONEDITIONABLE VIEW ""ART"".""ART_TI_ADVANCE_PAYMENT_UTILIZATION"" (""BRANCH"", ""ADV_REFERENCE"", ""PRINCIPAL_PARTY"", ""NON_PRINCIPAL_PARTY"", ""CREATION_DATE"", ""EXPIRY_DATE"", ""UTILIZATION_TRN"", ""ADV_AMOUNT"", ""ADV_CURRENCY"", ""UTILIZATION_AMOUNT"", ""UTILIZATION_CURRENCY"", ""OUTSTANDING_AMOUNT"") AS 
                          SELECT 
                        ADVMASTER.BHALF_BRN BRANCH,
                        ADVMASTER.MASTER_REF AS ""ADV_REFERENCE"" ,
                        PARTYDTLS.ADDRESS1 AS PRINCIPAL_PARTY,
                        (SELECT PARTYDTLS.ADDRESS1 FROM TIZONE2.PARTYDTLS@TIZONE2_LINK WHERE ADVMASTER.NPCP_PTY = PARTYDTLS.KEY97) AS NON_PRINCIPAL_PARTY,
                        ADVMASTER.CTRCT_DATE CREATION_DATE ,
                        ADVMASTER.EXPIRY_DAT EXPIRY_DATE,
                        LCDCMASTER.ORIG_REF UTILIZATION_TRN , 
                        EXTMASTERADP.AP_M_AMT ADV_AMOUNT ,
                        COALESCE(EXTMASTERADP.CCY, ' ') ADV_CURRENCY,
                        EXTMASTERADP.UTLS_AMT UTILIZATION_AMOUNT ,
                        COALESCE(EXTMASTERADP.CCY_4, ' ') UTILIZATION_CURRENCY ,
...
");
            ...
            CREATE OR REPLACE FORCE NONEDITIONABLE VIEW ""ART"".""ART_TI_ECM_WORKFLOW_PROG_REPORT"" (""ECM_REFERENCE"", ""ECM_CASE_CREATION_DATE"", ""BRANCH_ID"", ""CUTOMER_NAME"", ""PRODUCT"", ""PRODUCT_TYPE"", ""ECM_EVENT"", ""TRANSACTION_AMOUNT"", ""TRANSACTION_CURRENCY"", ""PRIMARY_OWNER"", ""CASE_STAT_CD"", ""UPDATE_USER_ID"", ""ECM_REJECTION_TYPE"", ""ECM_REJECTION_REASON"", ""FTI_REFERENCE"", ""EVENT_NAME"", ""EVENT_STATUS"", ""EVENT_CREATION_DATE"", ""ASSIGNED_TO"", ""ASSIGNMENT_TYPE"", ""EVENT_STEPS"", ""STEP_STATUS"", ""LSTMODTIME"", ""LSTMODUSER"", ""WARNING_OVERRIDDEN"", ""REJECTION_REASON"", ""SLA_DEADLINE"", ""INPUT_STEP_TIME"", ""INPUT_SLA_STATUS"", ""INPUT_MAX_TIME"", ""EXTERNAL_REVIEW_STEP_TIME"", ""EXTERNAL_REVIEW_SLA_STATUS"", ""REVIEW_STEP_TIME"", ""REVIEW_SLA_STATUS"", ""AUTHORIZE_STEP_TIME"", ""AUTHORIZE_SLA_STATUS"", ""SLA_DASHBOARD_AMBER"", ""SLA_DASHBOARD_RED"", ""SLA_REMAINING_TIME"") AS 
                                ...
                                prod.val_desc Product,
                                prodtype.val_desc Product_Type,
                                ...
                                ch.REJECTION_TYPE ECM_REJECTION_TYPE,
                                ch.REJECTION_REASON ECM_REJECTION_REASON,
                                ...
                                evstp.ASSNTOUNME Assigned_To,
                                (case
                                when ASSNTYPE='O' then 'Team set from originating message'
                                when ASSNTYPE='E' then 'Event / team mapping'
                                when ASSNTYPE='B' then 'Assigned to available team based on branch and transaction'
                                when ASSNTYPE='P' then 'Default team for product'
                                when ASSNTYPE='S' then 'SWIFT repair team for product'
                                when ASSNTYPE='G' then 'Gateway repair team for product'
                                when ASSNTYPE='M' then 'Manually assigned'
                                when ASSNTYPE='A' then 'Assigned to auto-allocate team based on branch and transaction'
                                when ASSNTYPE='R' then 'Responsible team'
                                when ASSNTYPE='U' then 'Assigned to current team / user based on step and team role'
                                when ASSNTYPE='T' then 'Assigned to current team based on step and team role'
                                when ASSNTYPE='C' then 'Default team for user'
                                when ASSNTYPE='D' then 'User profile responsible team'
                                end
                                ) Assignment_Type,
                                --eventstepnames.stepdescr,
                                (case
                                ...
                                evstp.LSTMODTIME,
                                evstp.LSTMODUSER,
                                warnmsg.OVERRIDDEN Warning_Overridden,
                                warnmsg.MESSAGE Rejection_Reason,
                                SLAEVTDATA.DEADLINE SLA_Deadline,
                                (SLAEVTDATA.INPUT_EFF/60) Input_Step_Time,
                                (CASE WHEN SLAEVTDATA.INPUT_EFF > SLAEVTDATA.INPUT_PROC THEN 'DELAYED' ELSE 'OK' END) AS Input_SLA_STATUS,
                                (SLAEVTDATA.INPUT_PROC/60) AS INPUT_MAX_TIME,
                                (SLAEVTDATA.ER_EFFORT/60) External_Review_Step_Time,
                                (CASE WHEN SLAEVTDATA.ER_EFFORT > SLAEVTDATA.ER_PROC THEN 'DELAYED' ELSE 'OK' END) AS External_Review_SLA_Status,
                                (SLAEVTDATA.RVW_EFFORT/60) Review_Step_Time,
                                (CASE WHEN SLAEVTDATA.RVW_EFFORT > SLAEVTDATA.RVW_PROC THEN 'DELAYED' ELSE 'OK' END) AS Review_SLA_Status,
                                (SLAEVTDATA.AUTHEFFORT/60) Authorize_step_Time,
                                (CASE WHEN SLAEVTDATA.AUTHEFFORT > SLAEVTDATA.AUTH_PROC then 'DELAYED' ELSE 'OK' END) AS Authorize_SLA_Status,
                                SLAEVTDATA.AMBERTIME SLA_Dashboard_Amber,
                                SLAEVTDATA.REDTIME SLA_Dashboard_Red,
                                SLAEVTDATA.REMAIN_C SLA_Remaining_TIme
                                --note.note_text Note,
                                --note.created_at Note_Creation_Time
                                ...
                                left join dgcmgmt.ref_table_val@DGDB_DGCMGMT prodtype on a.producttype=prodtype.val_cd
                                left join dgcmgmt.ref_table_val@DGDB_DGCMGMT event on a.eventname=event.val_cd
                                --left join DGECM_METADATA.COMMENTS@DGECM_METADATA_LINK comm on a.case_rk=comm.entity_rk
                                left join ACTIVITI_DB.WorkflowCaseHistory@DGDB_ACTIVITI_DB ch on a.case_rk=ch.caserk
                                ...
                                left join TIZONE2.EXEMPL30@TIZONE2_LINK evname on baseevent.exemplar = evname.key97
                                ...
                                --left join note on Note.master_key= mstr.key97
                                left join TIZONE2.warnmsg@TIZONE2_LINK on warnmsg.eventstep = evstp.key97
                                left join TIZONE2.SLAEVTDATA@TIZONE2_LINK on SLAEVTDATA.event = baseevent.key97") ;
            ...
            migrationBuilder.Sql($@"
             CREATE OR REPLACE FORCE NONEDITIONABLE VIEW ""ART"".""ART_TI_ECM_WORKFLOW_PROG_REPORT_OLD"" (""ECM_REFERENCE"", ""ECM_CASE_CREATION_DATE"", ""BRANCH_ID"", ""CUTOMER_NAME"", ""PRODUCT"", ""PRODUCT_TYPE"", ""ECM_EVENT"", ""TRANSACTION_AMOUNT"", ""TRANSACTION_CURRENCY"", ""PRIMARY_OWNER"", ""CASE_STAT_CD"", ""UPDATE_USER_ID"", ""COMMENTS"", ""ECM_REJECTION_TYPE"", ""ECM_REJECTION_REASON"", ""FTI_REFERENCE"", ""EVENT_NAME"", ""EVENT_STATUS"", ""EVENT_CREATION_DATE"", ""ASSIGNED_TO"", ""ASSIGNMENT_TYPE"", ""EVENT_STEPS"", ""STEP_STATUS"", ""LSTMODTIME"", ""LSTMODUSER"", ""WARNING_OVERRIDDEN"", ""REJECTION_REASON"", ""SLA_DEADLINE"", ""INPUT_STEP_TIME"", ""INPUT_SLA_STATUS"", ""INPUT_MAX_TIME"", ""EXTERNAL_REVIEW_STEP_TIME"", ""EXTERNAL_REVIEW_SLA_STATUS"", ""REVIEW_STEP_TIME"", ""REVIEW_SLA_STATUS"", ""AUTHORIZE_STEP_TIME"", ""AUTHORIZE_SLA_STATUS"", ""SLA_DASHBOARD_AMBER"", ""SLA_DASHBOARD_RED"", ""SLA_REMAINING_TIME"", ""NOTE"", ""NOTE_CREATION_TIME"") AS 
                        ...
                        left join TIZONE2.EVENTSTEP@TIZONE2_LINK evstp on BASEEVENT.KEY97=evstp.EVENT_KEY
                        left join TIZONE2.EVENTSTEPNAMES@TIZONE2_LINK on evstp.KEY97=EVENTSTEPNAMES.STEPKEY
                        left join TIZONE2.note@TIZONE2_LINK on Note.master_key= mstr.key97
                        left join TIZONE2.warnmsg@TIZONE2_LINK on warnmsg.eventstep = evstp.key97
                        left join TIZONE2.SLAEVTDATA@TIZONE2_LINK on SLAEVTDATA.event = baseevent.key97");
            ...
            migrationBuilder.Sql($@"
              CREATE OR REPLACE FORCE NONEDITIONABLE VIEW ""ART"".""ART_TI_FULL_JOURNAL_REPORT"" (""DATAITEM"", ""USERNAME"", ""MTCE_TYPE"", ""MCMTCETYPE"", ""MCUSERNAME"", ""AREA_CODE"", ""AREA"", ""JKEY"", ""DATETIME"", ""MCOWNER"", ""ENTRYTIMEUTC"", ""MCACTION"", ""MCNOTE"", ""DATA_AFTER"", ""DATABEFORE"") AS 
                                      SELECT 
                                    ALLJOURNAL.DATAITEM, 
                                    ALLJOURNAL.USERNAME, --Filter For User from table secage88;
                                    (case
                                    when ALLJOURNAL.MTCE_TYPE='I' then 'Add'
                                    when ALLJOURNAL.MTCE_TYPE='U' then 'Update'
                                    when ALLJOURNAL.MTCE_TYPE='D' then 'Delete'
                                    when ALLJOURNAL.MTCE_TYPE='E' then 'Error'
                                    when ALLJOURNAL.MTCE_TYPE='S' then 'Start'
                                    when ALLJOURNAL.MTCE_TYPE='N' then 'End'
                                    when ALLJOURNAL.MTCE_TYPE='W' then 'Warning'
                                    when ALLJOURNAL.MTCE_TYPE='P' then 'Approve'
                                    when ALLJOURNAL.MTCE_TYPE='J' then 'Reject'
                                    when ALLJOURNAL.MTCE_TYPE='R' then 'Return'
                                    when ALLJOURNAL.MTCE_TYPE='M' then 'Submit'
                                    end)MTCE_TYPE, 
                                    ALLJOURNAL.MCMTCETYPE, 
                                    ALLJOURNAL.MCUSERNAME, 
                                    ALLJOURNAL.AREA AREA_CODE, 
                                    (
                                    case
                                    when ALLJOURNAL.AREA='S' then 'Static'
                                    when ALLJOURNAL.AREA='E' then 'Security'
                                    when ALLJOURNAL.AREA='T' then 'Tailoring data'
                                    when ALLJOURNAL.AREA='W' then 'SWIFT data'
                                    when ALLJOURNAL.AREA='Y' then 'System control'
                                    when ALLJOURNAL.AREA='B' then 'Batch'
                                    when ALLJOURNAL.AREA='V' then 'Validation'
                                    when ALLJOURNAL.AREA='D' then 'Departmental Limits'
                                    when ALLJOURNAL.AREA='P' then 'Process'
                                    when ALLJOURNAL.AREA='Z' then 'Zone - maker checker'
                                    when ALLJOURNAL.AREA='R' then 'Branch - maker checker'
                                    when ALLJOURNAL.AREA='U' then 'Customer - maker checker'
                                    when ALLJOURNAL.AREA='M' then 'Maker checker action'
                                    end
                                    ) AREA,--Filter For Area Need to create table for it 
                                    ALLJOURNAL.JKEY, 
                                    ALLJOURNAL.DATETIME, -- Filter For Dates
                                    ALLJOURNAL.MCOWNER, 
                                    ALLJOURNAL.ENTRYTIMEUTC, 
                                    ALLJOURNAL.MCACTION, 
                                    ALLJOURNAL.MCNOTE, 
                                    ALLJOURNAL.DATA_AFTER, 
                                    ALLJOURNAL.DATABEFORE

                                    FROM   

                                    TIZONE2.ALLJOURNAL@TIZONE2_LINK ALLJOURNAL --left join ART_TI_JOURNAL_AREA b on alljournal.area = b.areacode

                                    WHERE  
                                    (ALLJOURNAL.AREA='E' OR ALLJOURNAL.AREA='R' OR ALLJOURNAL.AREA='U' OR ALLJOURNAL.AREA='Y' OR ALLJOURNAL.AREA='Z')");
