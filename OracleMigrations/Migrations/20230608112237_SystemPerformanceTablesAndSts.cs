...
{
    public partial class SystemPerformanceTablesAndSts : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {

            migrationBuilder.Sql($@"CREATE OR REPLACE FORCE EDITIONABLE VIEW ""ART"".""ART_SYSTEM_PERFORMANCE"" as
                                            select 
                                            a.CASE_ID, 
                                            ...
                                            a.investr_user_id investr_user_id,
                                            a.CREATE_DATE, 
                                            a.UPDATE_USER_ID,
                                            ...
                                            SELECT        
                                            /*+ PARALLEL */ 
                                            a.CASE_RK SWIFT_CASE_RK, 
                                            a.valid_from_date SWIFT_VALID_FROM_DTTM, 
                                            a.UDF_NAME, 
                                            a.udf_val AS X_SWFM
                                            FROM            
                                            dgcmgmt.case_udf_lgchr_val a RIGHT JOIN dgcmgmt.CASE_LIVE b 
                                            ON a.case_rk = b.case_rk AND a.valid_from_date = b.valid_from_date AND a.UDF_TABLE_NAME = 'CASE' AND UDF_NAME IN ('X_SWFM')
                                            ) SWIFT 
                                            ON a.CASE_RK = SWIFT.SWIFT_CASE_RK AND a.valid_from_date = SWIFT.SWIFT_VALID_FROM_DTTM LEFT JOIN
            ...
            migrationBuilder.Sql($@"CREATE OR REPLACE FORCE EDITIONABLE VIEW ""ART"".""ART_ALERTED_ENTITY"" as
                                        select 
...
");
        ...
        {
            migrationBuilder.Sql($@"DROP VIEW ""ART"".""ART_SYSTEM_PERFORMANCE""");
            migrationBuilder.Sql($@"DROP VIEW ""ART"".""ART_ALERTED_ENTITY""");
            migrationBuilder.Sql($@"DROP PROCEDURE ""ART"".""ART_ST_SYSTEM_PERF_PER_STATUS""");
            migrationBuilder.Sql($@"DROP PROCEDURE ""ART"".""ART_ST_SYSTEM_PERF_PER_TYPE""");
            migrationBuilder.Sql($@"DROP PROCEDURE ""ART"".""ART_ST_SYSTEM_PERF_PER_DATE""");
        }
    }
}
