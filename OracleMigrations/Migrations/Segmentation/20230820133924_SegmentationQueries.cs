...
--------------------------------------------------------
  ...
  SELECT 
      ...
      ,INDUSTRY_DESC
      ,OCCUPATION_DESC
      ,TO_CHAR (segment_sorted) segment_sorted
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW ""ART"".""ART_ALL_SEGMENT_CUST_COUNT"" (""MONTH_KEY"", ""SEGMENT_SORTED"", ""PARTY_TYPE_DESC"", ""NUMBER_OF_CUSTOMERS"", ""SEGMENT_DESCRIPTION"") AS 
  select 
Month_Key,
segment_sorted,
PARTY_TYPE_DESC,
count(party_number) Number_Of_Customers,
NVL(s.segment_description, '-')  segment_description
from ART.ART_MEB_SEGMENTS_V3_TB 
left join FCFKC.FSK_SEGMENT s on  segment_sorted = s.entity_segment_id
group by Month_Key,
segment_sorted,
PARTY_TYPE_DESC,
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW ""ART"".""ART_ALL_SEGS_FEATRS_STATCS"" AS  SELECT 
MONTH_KEY
,segment_sorted
,PARTY_TYPE_DESC      
,TO_CHAR(s.segment_description) segment_description
...
,FLOOR(MAX(MAX_FEES_D_AMT)) MAX_FEES_D_AMT
  ...
  left join FCFKC.FSK_SEGMENT s on  A.segment_sorted = s.entity_segment_id
...
;");
  ...
  select 
A.Month_Key,TO_CHAR(A.PARTY_NUMBER) PARTY_NUMBER ,party.party_name,
...
");
            migrationBuilder.Sql($@"  CREATE OR REPLACE FORCE EDITIONABLE VIEW ""ART"".""ART_ALERTS_PER_SEGMENT"" (""MONTH_KEY"", ""PARTY_TYPE_DESC"", ""SEGMENT_SORTED"", ""NUMBER_OF_ALERTS"", ""SEGMENT_DESCRIPTION"") AS 
  select 
month_key,
party_type_desc,
segment_sorted,
