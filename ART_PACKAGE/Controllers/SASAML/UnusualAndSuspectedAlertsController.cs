...
namespace ART_PACKAGE.Controllers.GOAML
{
    public class UnusualAndSuspectedAlertsController : Controller
    {
        ...
        public UnusualAndSuspectedAlertsController(SasAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_UNUSAL_ACTIVITIES> data = Enumerable.Empty<ART_ST_YEARLY_UNUSAL_ACTIVITIES>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_UNUSAL_ACTIVITIES>(SQLSERVERSPNames.ART_ST_YEARLY_UNUSAL_ACTIVITIES, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_UNUSAL_ACTIVITIES>(ORACLESPName.ART_ST_YEARLY_UNUSAL_ACTIVITIES, summaryParams.ToArray());
            }
            ...
            var result = new
            {
                ...
                total = Data.Total,
                reportname = "UnusualAndSuspectedAlerts"
            };
        ...
        public async Task<IActionResult> ExportPdf([FromBody] StoredReq para)
        {
            ...
            ViewData["title"] = "Unusual & Suspected Alerts Report";
            ViewData["desc"] = "";
