...
namespace DataGear_RV_Ver_1._7.Controllers
{
    ...
    {
        //private ModelContext db = new();
        //private readonly IHostingEnvironment _env;
        //public SASAuditListAppController(IHostingEnvironment env)
        //{
        //    this._env = env;
        //}
        ...
        public SASAuditListAppController(dbcmcaudit.ModelContext dbcmcaudit, IMemoryCache cache, IDropDownService dropDown, IHubContext<ExportHub> exportHub, ICsvExport csvSrv)
        {
        ...
        {
            IQueryable<VaLicensedView> data = dbcmcaudit.VaLicensedViews.AsQueryable();
            ...
            if (request.IsIntialize)
            {
                ...
                {
                    {"AppLebal".ToLower(),_dropDown.GetAppLebalDropDown().ToDynamicList() },
                    {"ProType".ToLower(),_dropDown.GetProTypeDropDown().ToDynamicList() },
                };
                ColumnsToSkip = ReportsConfig.CONFIG[nameof(SASAuditListAppController).ToLower()].SkipList;
            }


            var Data = data.CallData<VaLicensedView>(request, DropDownColumn, DisplayNames: DisplayNames, ColumnsToSkip);
            var result = new
            {
                data = Data.Data,
