...
{
    public class Bottom5SanctionBranchesController : Controller
    {
        ...
        public Bottom5SanctionBranchesController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES> data = Enumerable.Empty<ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES>(SQLSERVERSPNames.ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES>(ORACLESPName.ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.MySql)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES>(MYSQLSPName.ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            KendoDataDesc<ART_ST_YEARLY_BOTTOM_SANCTION_BRANCHES> Data = data.AsQueryable().CallData(para.req, columnsToDropDownd: DropDownColumn);
            ...
            var result = new
            {
                ...
                total = Data.Total,
                reportname = "Bottom5SanctionBranches"
            };
