...
{
    public class StaffGOAMLperRegionController : Controller
    {
        ...
        public StaffGOAMLperRegionController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION> data = Enumerable.Empty<ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION>(SQLSERVERSPNames.ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION>(ORACLESPName.ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            KendoDataDesc<ART_ST_YEARLY_STAFF_GOAML_AML_PER_REGION> Data = data.AsQueryable().CallData(para.req, columnsToDropDownd: DropDownColumn);
            ...
            var result = new
            {
                ...
                total = Data.Total,
                reportname = "StaffGOAMLperRegion"
            };
