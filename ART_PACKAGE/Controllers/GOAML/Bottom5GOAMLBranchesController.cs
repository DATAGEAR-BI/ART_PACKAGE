...
{
    public class Bottom5GOAMLBranchesController : Controller
    {
        ...
        public Bottom5GOAMLBranchesController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES> data = Enumerable.Empty<ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES>(SQLSERVERSPNames.ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES>(ORACLESPName.ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES, summaryParams.ToArray());
            }
            ...
            KendoDataDesc<ART_ST_YEARLY_BOTTOM_GOAML_BRANCHES> Data = data.AsQueryable().CallData(para.req);
            ...
            var result = new
            {
                ...
                total = Data.Total,
                reportname = "Bottom5GOAMLBranches"
            };
