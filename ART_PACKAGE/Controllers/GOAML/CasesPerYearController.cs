...
{
    public class CasesPerYearController : Controller
    {
        ...
        public CasesPerYearController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_SANCTION_PER_YEAR> data = Enumerable.Empty<ART_ST_YEARLY_SANCTION_PER_YEAR>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_YEAR>(SQLSERVERSPNames.ART_ST_YEARLY_SANCTION_PER_YEAR, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_YEAR>(ORACLESPName.ART_ST_YEARLY_SANCTION_PER_YEAR, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.MySql)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_YEAR>(MYSQLSPName.ART_ST_YEARLY_SANCTION_PER_YEAR, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            KendoDataDesc<ART_ST_YEARLY_SANCTION_PER_YEAR> Data = data.AsQueryable().CallData(para.req, columnsToDropDownd: DropDownColumn);
        ...
        public async Task<IActionResult> ExportPdf([FromBody] StoredReq para)
        {
            ...
            ViewData["title"] = "Cases Per Year Report";
            ViewData["desc"] = "";
