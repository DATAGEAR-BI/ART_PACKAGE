...
namespace ART_PACKAGE.Controllers.GOAML
{
    public class StaffGOAMLSANCTIONPerProductController : Controller
    {
        ...
        public StaffGOAMLSANCTIONPerProductController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_STAFF_GOAML_SANCTION_PER_PRODUCT> data = Enumerable.Empty<ART_ST_YEARLY_STAFF_GOAML_SANCTION_PER_PRODUCT>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_STAFF_GOAML_SANCTION_PER_PRODUCT>(SQLSERVERSPNames.ART_ST_YEARLY_STAFF_GOAML_SANCTION_PER_PRODUCT, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_STAFF_GOAML_SANCTION_PER_PRODUCT>(ORACLESPName.ART_ST_YEARLY_STAFF_GOAML_SANCTION_PER_PRODUCT, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            var result = new
            {
                ...
                total = Data.Total,
                reportname = "StaffGOAMLSANCTIONPerProduct"
            };
        ...
        public async Task<IActionResult> ExportPdf([FromBody] StoredReq para)
        {
            ...
            ViewData["title"] = "Staff GOAML SANCTION Per Product Report";
            ViewData["desc"] = "";
