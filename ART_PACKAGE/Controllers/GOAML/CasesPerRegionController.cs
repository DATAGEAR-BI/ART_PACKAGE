...
{
    public class CasesPerRegionController : Controller
    {
        ...
        public CasesPerRegionController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_SANCTION_PER_REGION> data = Enumerable.Empty<ART_ST_YEARLY_SANCTION_PER_REGION>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_REGION>(SQLSERVERSPNames.ART_ST_YEARLY_SANCTION_PER_REGION, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_REGION>(ORACLESPName.ART_ST_YEARLY_SANCTION_PER_REGION, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.MySql)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_REGION>(MYSQLSPName.ART_ST_YEARLY_SANCTION_PER_REGION, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            KendoDataDesc<ART_ST_YEARLY_SANCTION_PER_REGION> Data = data.AsQueryable().CallData(para.req, columnsToDropDownd: DropDownColumn);
        ...
        public async Task<IActionResult> ExportPdf([FromBody] StoredReq para)
        {
            ...
            ViewData["title"] = "Cases Per Region Report";
            ViewData["desc"] = "";
