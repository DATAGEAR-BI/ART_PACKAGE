...
{
    public class CasesPerProductController : Controller
    {
        ...
        public CasesPerProductController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_SANCTION_PER_PRODUCT> data = Enumerable.Empty<ART_ST_YEARLY_SANCTION_PER_PRODUCT>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_PRODUCT>(SQLSERVERSPNames.ART_ST_YEARLY_SANCTION_PER_PRODUCT, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_PRODUCT>(ORACLESPName.ART_ST_YEARLY_SANCTION_PER_PRODUCT, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.MySql)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_SANCTION_PER_PRODUCT>(MYSQLSPName.ART_ST_YEARLY_SANCTION_PER_PRODUCT, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            KendoDataDesc<ART_ST_YEARLY_SANCTION_PER_PRODUCT> Data = data.AsQueryable().CallData(para.req, columnsToDropDownd: DropDownColumn);
        ...
        public async Task<IActionResult> ExportPdf([FromBody] StoredReq para)
        {
            ...
            ViewData["title"] = "Cases Per Product Report";
            ViewData["desc"] = "";
