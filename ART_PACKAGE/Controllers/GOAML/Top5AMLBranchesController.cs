...
{
    public class Top5AMLBranchesController : Controller
    {
        ...
        public Top5AMLBranchesController(ArtGoAmlContext context, IPdfService pdfSrv, IDropDownService dropSrv, ICsvExport csvSrv, IConfiguration config)
        {
        ...
        {
            IEnumerable<ART_ST_YEARLY_TOP_AML_BRANCHES> data = Enumerable.Empty<ART_ST_YEARLY_TOP_AML_BRANCHES>().AsQueryable();

            IEnumerable<System.Data.Common.DbParameter> summaryParams = para.procFilters.MapToParameters(dbType);
            if (dbType == DbTypes.SqlServer)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_TOP_AML_BRANCHES>(SQLSERVERSPNames.ART_ST_YEARLY_TOP_AML_BRANCHES, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.Oracle)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_TOP_AML_BRANCHES>(ORACLESPName.ART_ST_YEARLY_TOP_AML_BRANCHES, summaryParams.ToArray());
            }
            else if (dbType == DbTypes.MySql)
            {
                data = context.ExecuteProc<ART_ST_YEARLY_TOP_AML_BRANCHES>(MYSQLSPName.ART_ST_YEARLY_TOP_AML_BRANCHES, summaryParams.ToArray());
            }
...
DropDownColumn = new Dictionary<string, List<dynamic>>
            ...
            KendoDataDesc<ART_ST_YEARLY_TOP_AML_BRANCHES> Data = data.AsQueryable().CallData(para.req, columnsToDropDownd: DropDownColumn);
        ...
        public async Task<IActionResult> ExportPdf([FromBody] StoredReq para)
        {
            ...
            ViewData["title"] = "Top 5 AML Branches Report";
            ViewData["desc"] = "";
