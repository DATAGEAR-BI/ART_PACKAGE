...
namespace ART_PACKAGE.Helpers.DropDown
{
    public class DropDownService : IDropDownService
    {
        ...
        public List<SelectItem> GetCaseTypesDropDown()
        {
            //var distinct_value = dbfcfcore.ScenarioNmMatviews.Select(x => x.ScenarioName).ToList();
            //return distinct_value;
            List<SelectItem> distinct_value = _dbSrv.ARTECM.ArtCaseTypesRefView.Where(s => s.CaseTypeCd != null).Select(x => x.CaseTypeCd).Select(x => new SelectItem { text = x, value = x }).ToList();
            return distinct_value;
        }
